...
import 'starsignscreen.dart';
import 'package:flutter_gen/gen_l10n/app_localizations.dart';
import 'package:moorky/constant/color.dart';
class ExtrovertIntrovertScreen extends StatefulWidget {
  bool isEdit=false;
  String introextrotext = "";
  bool back=false;
  bool skip=false;
  ExtrovertIntrovertScreen({required this.isEdit,required this.introextrotext,required this.back});
  @override
  State<ExtrovertIntrovertScreen> createState() => _ExtrovertIntrovertScreenState();
}

class _ExtrovertIntrovertScreenState extends State<ExtrovertIntrovertScreen> {
  ...
  void Init() async {
    ...
    {
      quizprovider.fetchIntroExtroList(preferences!.getString("accesstoken").toString());
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                        ...
                        child: addSemiBoldText(AppLocalizations.of(context)!.areyouintroextro,18, Colorss.mainColor)
                    ),
                    SizedBox(
                      height: 20.sp,
                    ),
                    Expanded(
                      child: Consumer<QuizProvider>(
                          builder: (context, quizprovider, child) => quizprovider.introextroList?.data != null?
                          ListView.builder(
                            itemCount: quizprovider.introextroList!.data!.length,
                            shrinkWrap: true,
                            scrollDirection: Axis.vertical,
                            physics: ScrollPhysics(),
                            itemBuilder: (BuildContext context, int index) {
                              for(int i=0;i<quizprovider.introextroList!.data!.length;i++)
                              {
                                if(quizprovider.introextroList!.data!.elementAt(index).name.toString()==widget.introextrotext)
                                {
                                  quizprovider.introextroList!.data!.elementAt(index).isSelected=true;
                                  introextro_id=quizprovider.introextroList!.data!.elementAt(index).id!;
                                }
                              }
                              return Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    addRegularText( quizprovider.introextroList!.data!.elementAt(index).name!, 12, Color(0xFF15294B)),
                                    Checkbox(
                                      shape: RoundedRectangleBorder(
                                          borderRadius: BorderRadius.all(
                                              Radius.circular(5.0.r))),
                                      activeColor: Colorss.mainColor,
                                      value: quizprovider.introextroList!.data!.elementAt(index).isSelected==null?false:quizprovider.introextroList!.data!.elementAt(index).isSelected,
                                      onChanged: (value) {
                                        setState(() {
                                          setState(() {
                                            for(int i=0;i<quizprovider.introextroList!.data!.length;i++)
                                            {
                                              if(quizprovider.introextroList!.data!.elementAt(i).isSelected==true)
                                              {
                                                quizprovider.introextroList!.data!.elementAt(i).isSelected=false;
                                              }
                                              widget.introextrotext=quizprovider.introextroList!.data!.elementAt(index).name!;
                                              introextro_id=quizprovider.introextroList!.data!.elementAt(index).id!;
                                              quizprovider.introextroList!.data!.elementAt(index).isSelected = value;
                                            }
                                          });
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            onBoardProgress(160.w),
                            widget.introextrotext!=""?GestureDetector(
                                onTap: ()async {
                                  setState(() {
                                    isLoad=true;
                                  });
                                  var model=await ProfileRepository.updateProfile(introextro_id.toString(), "introvert_or_extrovert", preferences!.getString("accesstoken")!);
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        onBoardProgress(160.w),
                        widget.introextrotext!=""?Row(
                          mainAxisAlignment: MainAxisAlignment.end,
                          children: [
                            GestureDetector(
                                onTap: () {
                                  preferences!.setString("introextrotext", introextro_id.toString());
                                  Navigator.push(context,
                                      MaterialPageRoute(builder:
                                          (context) =>
                                          StarSignScreen(isEdit: false,starsigntext: "",back: true,)
                                      )
                                  );
                                },
                                child:
