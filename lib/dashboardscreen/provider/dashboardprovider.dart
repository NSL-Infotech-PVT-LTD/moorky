...
class DashboardProvider extends ChangeNotifier{
  ...
  fetchUserList(String accessToken,int page,int limit,String age_from,String age_to,String date_with,String only_premium,String type,
      ...
      String star_sign,String have_pets,String religion,String languages,String refresh)
  async{
    UserModel? userList = await DashboardRepository.getUserList(accessToken,page,limit,age_from,age_to,date_with,only_premium,type,search,maritals,looking_fors,sexual_orientation,start_tall_are_you,end_tall_are_you,do_you_drink,do_you_smoke,feel_about_kids,education,introvert_or_extrovert,star_sign,have_pets,religion,languages,refresh);
  ...
  fetchSearcUserList(String accessToken,String user_id,int page,int limit)
  async{
    UserModel? userList = await DashboardRepository.getSaecrhUserList(accessToken,page,limit,"all",user_id);
  ...
  fetchMatchesUserList(String accessToken,int page,int limit,String type)
  async{
    List<UserDatum>? matchuserList = await DashboardRepository.getLikeUserList(accessToken,page,limit,type);
    if (_matchuserList == null) {
      _matchuserList = matchuserList;
    }
    notifyListeners();
  }

  fetchSearchUserList(String accessToken,String user_id,String type)
  async{
    List<UserDatum>? searchuserList = await DashboardRepository.getSearchUserList(accessToken,type,user_id);
  ...
  fetchFilterList(String accessToken)
  async{
    FilterListModel? filterListModel = await DashboardRepository.getFilterList(accessToken);
  ...
  fetchChatLists(String accessToken,String user_id)
  async{
    ChatListModel? chatList = await DashboardRepository.chatList(accessToken,user_id);
    if (_chatList == null) {
      _chatList = chatList;
  ...
  fetchRemainderList(String accessToken)
  async{
    RemainderListModel? remainderListModel = await DashboardRepository.getRemainderList(accessToken);
  ...
  fetchEventList(String accessToken,String userId)
  async{
    EventModelList? eventModelList = await DashboardRepository.getEventList(accessToken,userId);
    if (_eventModelList == null) {
      ...
      for(int i=0;i<eventModelList.data!.length;i++)
      {
        ...
        var inputDate = inputFormat.parse(
            eventModelList.data!.elementAt(i).date
            .toString());
        var neat= NeatCleanCalendarEvent(eventModelList.data!.elementAt(i).title,
            description: eventModelList.data!.elementAt(i).description,
            startTime: DateTime(inputDate.year, inputDate.month,
                inputDate.day),endTime: DateTime(inputDate.year, inputDate.month,
                inputDate.day),color: Colors.indigo);
        ev.add(neat);
      }
      event=ev;
      print("this is event");
  ...
  fetchGhostStickerList(String accessToken)
  async{
    GhostModel? ghostModel = await DashboardRepository.getGhostStickerList(accessToken);
    if (_ghostModel == null) {
  ...
  fetchChatList(String accessToken)
  async{
    print("sjhdcjhdcdsc");
    UsersChatListModel? usersChatListModel = await DashboardRepository.getChatList(accessToken);
  ...
  fetchUserActivity(String accessToken)
  async{
    YourActivitylistModel? yourActivitylistModel = await DashboardRepository.getUserAcitvity(accessToken);
  ...
  fetchLikeUserList(String accessToken,int page,int limit,String type)
  async{
    if((totalPages==0)||page<=totalPages) {
      List<UserDatum>? likeuserList = await DashboardRepository.getLikeUserList(accessToken,page,limit,type);
  ...
  fetchCampaignList(String accessToken)
  async{
    CampaignModel? campaignList = await DashboardRepository.getCampignList(accessToken);
    if (_campaignList == null) {
      _campaignList = campaignList;
    }
    notifyListeners();
  }
  fetchCampaignDetails(String accessToken,int id)
  async{
    CampaignDetailModel? campaigndetails = await DashboardRepository.getCampignDetails(accessToken,id);
