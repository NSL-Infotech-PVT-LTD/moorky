...
class ProfileProvider extends ChangeNotifier {
  ...
  void adddetails(ProfileDetailModel? model) {
    ...
    only_premium_member =
        model.data!.userfilterdata!.only_premium_member.toString();
    ...
    sexual_orientation =
        model.data!.userfilterdata!.sexual_orientation.toString();
    start_tall_are_you =
        model.data!.userfilterdata!.start_tall_are_you.toString();
    ...
    introvert_or_extrovert =
        model.data!.userfilterdata!.introvert_or_extrovert.toString();
  ...
  fetchMaritalList(String accessToken) async {
    _maritalstatusList = null;
    MaritalStatusModel? maritalstatusList =
        await ProfileRepository.getmaritalList(accessToken);
  ...
  fetchLookingforList(String accessToken) async {
    LookingForModel? lookingForList =
        await ProfileRepository.getlookingForList(accessToken);
  ...
  fetchIntrestedList(String accessToken) async {
    _interestedList = null;
    InterestedModel? interestedList =
        await ProfileRepository.getInterestedList(accessToken);
  ...
  fetchProfileDetailsuseractivity(String accessToken) async {
    ProfileDetailModel? profiledetails =
        await ProfileRepository.profileDetails(accessToken);

    _profiledetails = profiledetails;
    SharedPreferences preferences = await SharedPreferences.getInstance();
    preferences.setString("userName", profiledetails.data?.name??"");
    ...
    anotheruserImage =
        profiledetails.data!.active_monogamy_user_image.toString();
    ...
    only_premium_member =
        profiledetails.data!.userfilterdata!.only_premium_member.toString();
    ...
    sexual_orientation =
        profiledetails.data!.userfilterdata!.sexual_orientation.toString();
    start_tall_are_you =
        profiledetails.data!.userfilterdata!.start_tall_are_you.toString();
    end_tall_are_you =
    ...
    introvert_or_extrovert =
        profiledetails.data!.userfilterdata!.introvert_or_extrovert.toString();
  ...
  fetchProfileDetails(String accessToken) async {
    ProfileDetailModel? profiledetails =
        await ProfileRepository.profileDetails(accessToken);
    if (_profiledetails == null) {
      ...
      isChat = profiledetails.data!.active_monogamy!;
      isEvent = profiledetails.data!.active_monogamy!;
      prefs.setString("email", profiledetails?.data?.email??"");

      prefs.setString("profileImage", profileImage);
      print("shjsshsh${profiledetails?.data?.id}");
      anotheruserID = profiledetails.data!.active_monogamy_user_id.toString();
      isSubscriptionUser = profiledetails.data!.subscription!;
      user_type = profiledetails.data!.user_plan!;
      swipecount = profiledetails.data!.swipe_count!;
      ...
      anotheruserImage =
          profiledetails.data!.active_monogamy_user_image.toString();
      anotheruserName =
          profiledetails.data!.active_monogamy_user_name.toString();
      direct_chat_count = profiledetails.data!.direct_chat_count;
      activemonogomy = profiledetails.data!.active_monogamy!;
      datewith = profiledetails.data!.userfilterdata!.date_with.toString();
      if (datewith == "") {
  ...
  fetchUserProfileDetails(String accessToken, String user_id) async {
    ProfileDetailModel? userprofiledetails =
        await ProfileRepository.userprofileDetails(accessToken, user_id);
  ...
  fetchUserReportReason(String accessToken) async {
    var reportJson= {"statusCode":200,"message":"Report report-reasons found successfully.","data":[{"id":1,"reason":"Unknown","status":true,"created_at":"2022\/08\/23 06:45:10","updated_at":"2022\/08\/23 06:45:10"},{"id":2,"reason":"Abused","status":true,"created_at":"2022\/08\/19 05:15:43","updated_at":"2022\/08\/19 05:15:43"},{"id":3,"reason":"Fraud","status":true,"created_at":"2022\/08\/23 06:45:10","updated_at":"2022\/08\/23 06:45:10"}]};
    _reportReasonListModel=  ReportReasonListModel.fromJson(reportJson);
    notifyListeners();
    ReportReasonListModel? reportReasonListModel =
        await ProfileRepository.userreportReason(accessToken);
  ...
  getQuizQuestion(ProfileDetailModel model) {
    ...
    if (model.data!.school != "") {
      ...
      if (quizquestion == "") {
        quizquestion = model.data!.quizQuestion!.school;
      }
    }
    ...
    if (model.data!.userQuestions!.length > 0) {
    } else {
      if (quizquestion == "") {
  ...
  Future<bool>userChatOrNot({required String userId, required String otherUserId,required String accessToken}) async{
   bool response =
        await ProfileRepository().userChatOrNot(accesstoken: accessToken,userId: userId,otherUserId: otherUserId);
   notifyListeners();
    return response;
